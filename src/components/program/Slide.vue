<template lang="">
    <div class="p-2 my-2 rounded-lg border-2 transition-all duration-300">
        <div class="flex flex-row items-center p-2">
            <h1 class="text-2xl text-center">{{slide.title}}</h1>
            <div class="grow"></div>
        ...
        <OldHymnalEdit :class="[`transition-max  duration-300 overflow-hidden ${editableSize}`]" :data="slide.data"/>
    </div>
</template>
...
export default {
    ...
    computed: {
        editableSize(){
            if(this.editable){
                return editableSizes.find(slide => slide.slideType === this.slide.type).size
            }
            return 'h-0'
        }
    },
